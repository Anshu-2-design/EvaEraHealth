...
function buildStepContent(id) {
  ...
  if (id === 'psychosexual')        return buildPsychosexual(a);
...
function buildDemographics(a) {
  ...
  html += '<input class="text-input" placeholder="e.g. Delhi, Mumbai" value="'+(a.city||'')+'" oninput="setAns(\'city\',this.value)"></div>';
  ...
  if (a.bmi) setTimeout(function(){var d=document.getElementById('bmi-display');if(d)d.style.display='block';},10);
  html += '<div class="field-group"><label class="field-label">Menstrual Status</label><div class="cards">';
  [['🔴','Perimenopause','Irregular cycles'],['🌸','Menopause (<1yr)','No period < 1 year'],['🌺','Post-Menopause','No period > 1 year'],['🌙','Surgical Menopause','After surgery/chemo']].forEach(function(opt){
  ...
  ['Married','Partner','Single','Separated/Divorced','Widowed','Prefer not to say'].forEach(function(opt){
    html += '<div class="card-opt '+(a.marital===opt?'selected':'')+'" onclick="toggleCard(\'marital\',\''+opt+'\',this)">'+opt+'</div>';
  });
  html += '</div></div>';
  ...
  ['Up to 12th','Graduate','Post-Graduate','Doctorate','Prefer not to say'].forEach(function(opt){
    html += '<div class="card-opt '+(a.education===opt?'selected':'')+'" onclick="toggleCard(\'education\',\''+opt+'\',this)">'+opt+'</div>';
  });
  html += '</div></div>';
  return html;
}
...
function buildGatePsych(a) {
  ...
  return '<div class="gate-card"><div style="font-size:40px;text-align:center;margin-bottom:12px">🧠</div>' +
    ...
    '<div class="gate-btns"><button class="gate-btn-yes" onclick="gateChoice(\'gate_psych\',\'yes\')">Yes, continue</button>' +
    '<button class="gate-btn-no" onclick="gateChoice(\'gate_psych\',\'no\')">Skip for now</button></div></div>';
}
...
function buildGateSexual(a) {
  ...
  return '<div class="gate-card"><div style="font-size:40px;text-align:center;margin-bottom:12px">💜</div>' +
    ...
    '<div class="gate-btns"><button class="gate-btn-yes" onclick="gateChoice(\'gate_sexual\',\'yes\')">Yes, continue</button>' +
    '<button class="gate-btn-no" onclick="gateChoice(\'gate_sexual\',\'no\')">Skip for now</button></div></div>';
}

function buildGateSleep(a) {
  var isi   = S.answers._isi_score || 0;
  var band  = isi < 8 ? 'Minimal' : isi < 15 ? 'Moderate' : 'Severe';
  ...
  return '<div class="gate-card"><div style="font-size:40px;text-align:center;margin-bottom:12px">🌙</div>' +
    ...
    '<div class="gate-btns"><button class="gate-btn-yes" onclick="gateChoice(\'gate_sleep\',\'yes\')">Yes, include it</button>' +
    '<button class="gate-btn-no" onclick="gateChoice(\'gate_sleep\',\'no\')">Continue without</button></div></div>';
}

/*MENTAL HEALTH (PHQ9 + GAD7 + PSS8)*/
function buildMentalHealth(a) {
  var html     = '<div class="section-label">PHQ-9 — Depression Screen</div>';
  var phqQs    = ['Little interest or pleasure in doing things','Feeling down, depressed, or hopeless','Trouble falling or staying asleep','Feeling tired or having little energy','Poor appetite or overeating','Feeling bad about yourself','Trouble concentrating','Moving or speaking too slowly, or being fidgety','Thoughts that you would be better off dead or of hurting yourself'];
  var phqOpts  = ['Not at all','Several days','More than half the days','Nearly every day'];
  phqQs.forEach(function(q, i) {
    html += '<div class="likert-item"><div class="likert-q">'+(i+1)+'. '+q+(i===8?' <strong style="color:var(--danger)">[Please answer carefully]</strong>':'')+'</div><div class="radio-group" style="gap:6px">';
    phqOpts.forEach(function(opt, v) {
      html += '<div class="radio-opt '+(a['phq_'+i]===v?'selected':'')+'" onclick="setRadio(\'phq_'+i+'\','+v+',this)"><input type="radio" '+(a['phq_'+i]===v?'checked':'')+' readonly><label>'+opt+'</label></div>';
    });
    html += '</div></div>';
  });
  html += '<div class="section-label" style="margin-top:24px">GAD-7 — Anxiety Screen</div>';
  ['Feeling nervous, anxious, or on edge','Not being able to stop or control worrying','Worrying too much about different things','Trouble relaxing','Being so restless that it is hard to sit still','Becoming easily annoyed or irritable','Feeling afraid, as if something awful might happen'].forEach(function(q, i) {
    html += '<div class="likert-item"><div class="likert-q">'+(i+1)+'. '+q+'</div><div class="radio-group" style="gap:6px">';
    phqOpts.forEach(function(opt, v) {
      html += '<div class="radio-opt '+(a['gad_'+i]===v?'selected':'')+'" onclick="setRadio(\'gad_'+i+'\','+v+',this)"><input type="radio" '+(a['gad_'+i]===v?'checked':'')+' readonly><label>'+opt+'</label></div>';
    });
    html += '</div></div>';
  });
  ...
  pssQs.forEach(function(q, i) {
    ...
    for (var v = 0; v <= 4; v++) {
      html += '<button class="likert-btn '+(a['pss_'+i]===v?'selected':'')+'" onclick="setLikert(\'pss_'+i+'\','+v+',this)" title="'+pssOpts[v]+'">'+v+'</button>';
    }
...
function buildSleep(a) {
  var qs = [
    ...
    {k:'isi_5',q:'How worried / distressed are you about your sleep problem?',labels:['Not at all','A little','Somewhat','Much','Very Much']},
  ...
  qs.forEach(function(q) {
    html += '<div class="likert-item"><div class="likert-q">'+q.q+'</div><div class="likert-scale">';
    for (var v = 0; v <= 4; v++) {
      html += '<button class="likert-btn '+(a[q.k]===v?'selected':'')+'" onclick="setLikert(\''+q.k+'\','+v+',this)" title="'+q.labels[v]+'">'+v+'</button>';
    }
...
/*PSYCHOSEXUAL (FSFI + FSDSR + MCSS)*/
function buildPsychosexual(a) {
  var html = '<div style="background:var(--purple-lt);border:1px solid rgba(106,27,154,0.2);border-radius:10px;padding:12px 14px;margin-bottom:16px;font-size:12px;color:var(--purple)">🔒 Strictly confidential. DPDP Act 2023.</div>';
  html += '<div class="section-label">FSFI — Sexual Function (past 4 weeks)</div>';
  var fsfiQs = ['How often did you feel sexual desire or interest?','How would you rate your level of sexual desire?','How often did you feel sexually aroused during sexual activity?','How would you rate your level of sexual arousal?','How confident were you about becoming sexually aroused?','How often were you satisfied with your arousal?','How often did you become lubricated (wet)?','How difficult was it to become lubricated (wet)?','How often did you maintain your lubrication to completion?','How difficult was it to maintain lubrication to completion?','How often did you reach orgasm?','How difficult was it to reach orgasm?','How satisfied were you with your ability to reach orgasm?','How satisfied with emotional closeness during sex?','How satisfied with your sexual relationship?','How satisfied with your overall sex life?','How often did you experience pain during vaginal penetration?','How often did you experience pain following vaginal penetration?','How would you rate your level of pain during or following penetration?'];
  fsfiQs.forEach(function(q, i) {
    html += '<div class="likert-item"><div class="likert-q">'+(i+1)+'. '+q+'</div><div class="likert-scale">';
    for (var v = 0; v <= 5; v++) {
      html += '<button class="likert-btn '+(a['fsfi_'+i]===v?'selected':'')+'" onclick="setLikert(\'fsfi_'+i+'\','+v+',this)">'+v+'</button>';
    }
    html += '</div><div class="likert-labels"><span>Never/No function</span><span>Almost always</span></div></div>';
  });
  html += '<div class="section-label" style="margin-top:24px">FSDSR — Sexual Distress</div>';
  ['Distressed about your sex life','Unhappy about your sexual relationship','Dissatisfied with your sex life','Unhappy about how often you have sex','Worried your sexual desire is too low','Experiencing a problem with sexual desire','Self-conscious about your sexuality','Troubled by lack of sexual interest','Bothered by lack of sexual pleasure','Personally distressed by sexual difficulties','Bothered by low sexual desire','Upset by low sexual desire','Concerned about your sexual difficulties'].forEach(function(q, i) {
    html += '<div class="likert-item"><div class="likert-q">'+(i+1)+'. '+q+'</div><div class="likert-scale">';
    for (var v = 0; v <= 4; v++) {
      html += '<button class="likert-btn '+(a['fsdsr_'+i]===v?'selected':'')+'" onclick="setLikert(\'fsdsr_'+i+'\','+v+',this)">'+v+'</button>';
    }
    html += '</div><div class="likert-labels"><span>Never</span><span>Always</span></div></div>';
  });
  html += '<div class="section-label" style="margin-top:24px">Relationship Wellbeing</div>';
  ['How satisfied are you with your current intimate relationship?','How well does your partner understand your menopause-related changes?','How much does your intimate relationship affect your symptoms?','How supported do you feel by your partner regarding your health?','Overall, how would you rate the quality of your intimate relationship?'].forEach(function(q, i) {
    html += '<div class="likert-item"><div class="likert-q">'+(i+1)+'. '+q+'</div><div class="likert-scale">';
    for (var v = 0; v <= 4; v++) {
      html += '<button class="likert-btn '+(a['mcss_'+(i+1)]===v?'selected':'')+'" onclick="setLikert(\'mcss_'+(i+1)+'\','+v+',this)">'+v+'</button>';
    }
    html += '</div><div class="likert-labels"><span>Very Poor</span><span>Very Good</span></div></div>';
  });
  return html;
}
...
function buildPrakriti(a) {
  ...
  list.forEach(function(p) {
    html += '<div class="card-opt '+(a.prakriti===p.v?'selected':'')+'" onclick="toggleCard(\'prakriti\',\''+p.v+'\',this)" style="text-align:left;padding:16px 18px">';
...
function buildVikriti(a) {
  ...
  list.forEach(function(v) {
    html += '<div class="vikriti-card '+(a.vikriti===v.v?'selected':'')+'" onclick="setAns(\'vikriti\',\''+v.v+'\');this.parentElement.querySelectorAll(\'.vikriti-card\').forEach(function(c){c.classList.remove(\'selected\')});this.classList.add(\'selected\')">';
...
function buildWearableData(a) {
  ...
  html += '<div style="background:linear-gradient(135deg,#E0F7FA,#F0FDF4);border-radius:12px;padding:14px 16px;margin-bottom:14px"><p style="font-size:12px;font-weight:700;color:var(--teal)">📊 Enter 30-day averages from your device app</p><p style="font-size:11px;color:var(--slate)">All fields optional — enter what is available in your app</p></div>';
  html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">';
  [{k:'avg_rhr',l:'Resting Heart Rate',u:'bpm',ph:'e.g. 72',h:'Normal: 60–80 bpm'},
    ...
    {k:'avg_spo2',l:'Skin Temp Variation',u:'°C',ph:'e.g. 0.5',h:'Normal: <1°C variation'}
  ].forEach(function(f) {
    html += '<div style="background:#fff;border:1.5px solid #E5E7EB;border-radius:10px;padding:12px">';
    html += '<div style="font-size:10px;font-weight:700;color:var(--slate)">'+f.l+' <span style="color:var(--rose)">'+f.u+'</span></div>';
...
function buildComorbidities(a) {
  ...
  var conditions = [{n:'Hypertension',icon:'❤️'},{n:'Diabetes',icon:'🩸'},{n:'Hypothyroidism',icon:'🦋'},{n:'Hyperthyroidism',icon:'⚡'},{n:'Hyperlipidemia',icon:'💊'},{n:'Anaemia',icon:'🩺'},{n:'PCOD',icon:'🌿'},{n:'Osteoporosis',icon:'🦴'},{n:'Heart Disease',icon:'🫀'},{n:'CKD',icon:'💧'},{n:'Autoimmune Disorder',icon:'🛡️'},{n:'Stroke (history)',icon:'🧠'},{n:'Cancer (history)',icon:'🎗️'}];
  var opts = ['No','Controlled','Uncontrolled','Not Sure'];
  var cols = {No:'#2E7D32',Controlled:'#E65100',Uncontrolled:'#C62828','Not Sure':'#5C6BC0'};
  var html = '<p style="font-size:12px;color:var(--slate);margin-bottom:14px">Select your current status for each condition. Choose Not Sure if uncertain — your clinician will clarify during the consultation.</p><div style="display:grid;gap:10px">';
